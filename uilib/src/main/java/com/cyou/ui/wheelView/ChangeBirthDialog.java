...
public class ChangeBirthDialog extends Dialog implements View.OnClickListener {
    ...
    private void initWvDay(){
        initDays(day);
        if(arry_days.size() <= currentDay){
            currentDay = Integer.valueOf(arry_days.get(arry_days.size() - 1));
        }else{
            currentDay = Integer.valueOf(arry_days.get(currentDay - 1));
        }
        mDaydapter = new CalendarTextAdapter(context, arry_days, currentDay-1, maxTextSize, minTextSize);
        wvDay.setVisibleItems(5);
        wvDay.setViewAdapter(mDaydapter);
